...
# Criar diretório para scripts auxiliares
echo -e "${VERDE}Criando diretório para scripts auxiliares...${RESET}"
mkdir -p mautic_scripts${SUFFIX}

# CORREÇÃO 1: Atualizar o script run_mautic_crons.sh para incluir o comando campaigns:update
...
chmod +x "mautic_scripts${SUFFIX}/run_mautic_crons.sh"
...
services:
  ...
  # CORREÇÃO 2: Atualizar o command do mautic-worker para incluir a fila "default"
  mautic-worker:
    image: mautic/mautic:latest
    networks:
      - GrowthNet
  ...
  # CORREÇÃO 3: Melhorar a robustez do mautic-cron adicionando php-cli à instalação
  mautic-cron:
    image: mautic/mautic:latest
    networks:
      - GrowthNet
    ...
    volumes:
      - mautic_data${SUFFIX}:/var/www/html
      - ./mautic_scripts${SUFFIX}/run_mautic_crons.sh:/usr/local/bin/run_mautic_crons.sh
    entrypoint: |
      bash -c '
...
# CORREÇÃO 4: Adicionar detecção do container real do Mautic e gerar comandos cron externos
