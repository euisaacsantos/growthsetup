...
services:
  ...
  # CORREÇÃO 3: Melhorar a robustez do mautic-cron adicionando php-cli
  mautic-cron:
    image: mautic/mautic:latest
    networks:
      - GrowthNet
...
echo -e "  ${BEGE}docker exec -it \$(docker ps -q --filter name=${MAUTIC_STACK_NAME}_mautic) php /var/www/html/bin/console mautic:campaigns:update${RESET}"
